...
`).join("")}function F(t){const r=v(t);d.insertAdjacentHTML("beforeend",r)}const P=document.querySelector(".search-form"),d=document.querySelector(".gallery"),L=document.querySelector(".loader"),u=document.querySelector(".btn-load");document.querySelector(".gallery-item");let c,n=1,b=0;const q=15;let g=new w(".lightbox a",{captionsData:"alt"});g.on("show.simplelightbox",function(){});m();setTimeout(h,100);P.addEventListener("submit",B);u.addEventListener("click",C);async function B(t){if(t.preventDefault(),p(),m(),d.innerHTML="",c=t.target.elements.search.value.trim(),n=1,!c){l.show({message:"Please fill in the field!",theme:"dark",progressBarColor:"#FFFFFF",color:"#EF4040",position:"topRight"}),h();return}let r;try{r=await y(c,n),b=Math.ceil(r.totalHits/q),g.refresh(),r.hits.length===0?l.error({maxWidth:"432px",height:"48px",theme:"dark",progressBarColor:"#FFFFFF",color:"#EF4040",position:"topRight",message:"Sorry, there are no images matching your search query. Please try again!"}):F(r.hits)}catch{l.error({maxWidth:"432px",height:"48px",theme:"dark",progressBarColor:"#FFFFFF",color:"#EF4040",position:"topRight",message:"Sorry, an error occurred while loading. Please try again!"})}h(),r.hits.length!==0&&x(),t.target.reset()}async function C(){n+=1,p(),m();try{const t=await y(c,n);F(t.hits),g.refresh()}catch{l.error({maxWidth:"432px",height:"48px",message:"Sorry, an error occurred while loading. Please try again!",theme:"dark",progressBarColor:"#FFFFFF",color:"#EF4040",position:"topRight"})}h(),E(),x()}function m(){L.classList.remove("hidden")}function h(){L.classList.add("hidden")}function k(){u.classList.remove("hidden")}function p(){u.classList.add("hidden")}function x(){n>=b?(p(),l.show({maxWidth:"432px",height:"48px",message:"We're sorry, but you've reached the end of search results.",theme:"dark",progressBarColor:"#FFFFFF",color:"blue",position:"topRight"})):k()}function E(){const t=d.getBoundingClientRect().height;scrollBy({top:t,behavior:"smooth"})}
